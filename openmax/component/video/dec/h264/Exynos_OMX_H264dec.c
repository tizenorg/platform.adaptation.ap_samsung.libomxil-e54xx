    ...
    OMX_U32            nOutbufs)
{
    ...
    nAllocLen[0] = CHOOSE_MAX_SIZE(
			calc_yplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                              pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight),
                        new_calc_yplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                              pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight));

    nAllocLen[1] = CHOOSE_MAX_SIZE(
			calc_uvplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
				pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight >> 1),
                        new_calc_uvplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                                pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight >> 1));
...
OMX_ERRORTYPE H264CodecDstSetup(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    nAllocLen[0] = CHOOSE_MAX_SIZE(calc_yplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                                       pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight),
                                   new_calc_yplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                                       pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight));

    nAllocLen[1] = CHOOSE_MAX_SIZE(calc_uvplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                                       pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight >> 1),
                                   new_calc_uvplane(pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth,
                                       pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight >> 1));
...
OMX_ERRORTYPE H264CodecCheckResolutionChange(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    Exynos_OSAL_Log(EXYNOS_LOG_TRACE, "H264CodecCheckResolutionChange H264CodecSetup nOutbufs: %d", pH264Dec->hMFCH264Handle.maxDPBNum);
    ...
    if (((pVideoDec->bDRCProcessing) && (pExynosOutputPort->bufferProcessType & BUFFER_COPY)) ||
        ...
        /* Check Crop */
        pExynosInputPort->portDefinition.format.video.nFrameWidth = pH264Dec->hMFCH264Handle.codecOutbufConf.cropRect.nWidth;//.nFrameWidth;
        pExynosInputPort->portDefinition.format.video.nFrameHeight = pH264Dec->hMFCH264Handle.codecOutbufConf.cropRect.nHeight;//.nFrameHeight;
        pExynosInputPort->portDefinition.format.video.nStride = ((pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameWidth + 15) & (~15));
        pExynosInputPort->portDefinition.format.video.nSliceHeight = ((pH264Dec->hMFCH264Handle.codecOutbufConf.nFrameHeight + 15) & (~15));
        Exynos_UpdateFrameSize(pOMXComponent);
