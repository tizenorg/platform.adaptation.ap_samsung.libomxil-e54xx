...
OMX_ERRORTYPE Mpeg2CodecSrcInit(OMX_COMPONENTTYPE *pOMXComponent)
{
    OMX_ERRORTYPE                  ret = OMX_ErrorNone;
    ...
    if (pExynosInputPort->bufferProcessType & BUFFER_SHARE
        ||pMpeg2Dec->hMFCMpeg2Handle.bShareableBuf == OMX_TRUE)
    ...
    if (((pExynosInputPort->bufferProcessType & BUFFER_COPY) == BUFFER_COPY) &&
        ...
        pMpeg2Dec->hMFCMpeg2Handle.bShareableBuf == OMX_FALSE) {
        ...
        for (i = 0; i < MFC_INPUT_BUFFER_NUM_MAX; i++) {
            ...
            if (pInbufOps->Get_Buffer) {
                if (pInbufOps->Get_Buffer(pMpeg2Dec->hMFCMpeg2Handle.hMFCHandle, i, &pBuffer) != VIDEO_ERROR_NONE) {
...
//        ret = OMX_ErrorInsufficientResources;
    ...
    /* check for resolution change and update dstoutbuf count */
...
OMX_ERRORTYPE Mpeg2CodecDstSetup(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    nAllocLen[0] = CHOOSE_MAX_SIZE(
                       calc_yplane(pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameWidth,
                               pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameHeight),
                       new_calc_yplane(pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameWidth,
                               pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameHeight)
                  );

    nAllocLen[1] = CHOOSE_MAX_SIZE(
                     calc_uvplane(pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameWidth,
                             pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameHeight >> 1),
                     new_calc_uvplane(pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameWidth,
                             pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameHeight >> 1)
                  );


    /* MFC FIX. double up calculated plane size */
    nAllocLen[0] <<= 1;
    nAllocLen[1] <<= 1;
...
OMX_ERRORTYPE Mpeg2CodecCheckResolutionChange(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    pExynosOutputPort->cropRectangle.nTop = pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.cropRect.nTop;
    pExynosOutputPort->cropRectangle.nLeft = pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.cropRect.nLeft;
    pExynosOutputPort->cropRectangle.nWidth = pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.cropRect.nWidth;
    pExynosOutputPort->cropRectangle.nHeight = pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.cropRect.nHeight;
...
/*            pExynosOutputPort->exceptionFlag = NEED_PORT_DISABLE;*/
    ...
    if (((pVideoDec->bDRCProcessing) && (pExynosOutputPort->bufferProcessType & BUFFER_COPY)) ||
        ...
        /* Check Crop */
        pExynosInputPort->portDefinition.format.video.nFrameWidth = pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.cropRect.nWidth;
        pExynosInputPort->portDefinition.format.video.nFrameHeight = pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.cropRect.nHeight;
        pExynosInputPort->portDefinition.format.video.nStride = ((pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameWidth + 15) & (~15));
        pExynosInputPort->portDefinition.format.video.nSliceHeight = ((pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.nFrameHeight + 15) & (~15));
        Exynos_UpdateFrameSize(pOMXComponent);
...
OMX_ERRORTYPE Exynos_Mpeg2Dec_Init(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if ((pExynosInputPort->bufferProcessType & BUFFER_COPY) == BUFFER_COPY) {
        ...
        if (pMpeg2Dec->hMFCMpeg2Handle.bShareableBuf == OMX_TRUE) {
            for (i = 0; i < MFC_INPUT_BUFFER_NUM_MAX; i++) {
                pVideoDec->pMFCDecInputBuffer[i] = Exynos_OSAL_Malloc(sizeof(CODEC_DEC_BUFFER));
                Exynos_OSAL_Memset(pVideoDec->pMFCDecInputBuffer[i], 0, sizeof(CODEC_DEC_BUFFER));
                Exynos_OSAL_Log(EXYNOS_LOG_TRACE, "pVideoDec->pMFCDecInputBuffer[%d]: 0x%x", i, pVideoDec->pMFCDecInputBuffer[i]);
