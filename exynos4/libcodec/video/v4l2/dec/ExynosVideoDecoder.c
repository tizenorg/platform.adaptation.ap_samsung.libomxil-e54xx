...
#ifdef V4L2_CID_MPEG_MFC_BASE
#ifndef V4L2_CID_MPEG_VIDEO_DECODER_IMMEDIATE_DISPLAY
...
static void *MFC_Decoder_Init(int nMemoryType)
{
    ...
    needCaps = (V4L2_CAP_VIDEO_M2M_MPLANE | V4L2_CAP_STREAMING);
    ...
    unsigned int  nBufferCount)
{
    ...
    ALOGE("\n%s: Inbuf set up: shared=[%d] memory type=%d\n", __func__, pCtx->bShareInbuf,req.memory);
    ...
    for (nIndex = 0; nIndex < pCtx->nInbufs; nIndex++) {
        if (pCtx->pInbuf[nIndex].bRegistered == VIDEO_FALSE) {
            for (plane = 0; plane < nPlanes; plane++) {
                ...
                ALOGV("%s: registered buf %2d (addr=%p alloc_sz=%ld fd=%d)\n", __func__, nIndex,
    ...
    int               nPlanes)
{
    ...
    for (nIndex = 0; nIndex < pCtx->nOutbufs; nIndex++) {
        if (pCtx->pOutbuf[nIndex].bRegistered == VIDEO_FALSE) {
            ...
            ALOGV("%s: registered buf %2d plane[0]:(addr=%p alloc_sz=%d fd=%d) plane[1]:(addr=%p alloc_sz=%d fd=%d)\n",
    ...
    void          *pPrivate)
{
    ...
    memset(&buf, 0, sizeof(buf));
    memset(planes, 0, sizeof(struct v4l2_plane) * VIDEO_DECODER_INBUF_PLANES);

    buf.type = V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE;
    buf.m.planes = planes;
    buf.length = VIDEO_DECODER_INBUF_PLANES;
    buf.m.planes[0].bytesused = dataSize[0];
    pMutex = (pthread_mutex_t*)pCtx->pInMutex;
    pthread_mutex_lock(pMutex);
    index = MFC_Decoder_Find_Inbuf(pCtx, pBuffer[0]);
    if (index == -1) {
        pthread_mutex_unlock(pMutex);
        ALOGV("%s: Failed to get index", __func__);
        ret = VIDEO_ERROR_NOBUFFERS;
        goto EXIT;
    }
    ...
    void          *pPrivate)
{
    ...
    memset(&buf, 0, sizeof(buf));
#ifdef EXYNOS_4212
    memset(planes, 0, sizeof(struct v4l2_plane) * VIDEO_DECODER_OUTBUF_PLANES);
#endif
    ...
    if (index == -1) {
        pthread_mutex_unlock(pMutex);
        ALOGV("%s: Failed to get index", __func__);
        ret = VIDEO_ERROR_NONE;/*VIDEO_ERROR_NOBUFFERS;*/
        goto EXIT;
    }
    buf.index = index;
    ...
    if (pCtx->bShareOutbuf == VIDEO_TRUE) {
        ...
        for (i = 0; i < nPlanes; i++) {
            ...
            if (buf.m.planes[i].m.fd)
               buf.m.planes[i].length = 0;

#endif
            ALOGV(" plane=%d fd=%d addr=0x%lx bytes used=%d",i,
                  buf.m.planes[i].m.fd,
                  buf.m.planes[i].m.userptr,
                  buf.m.planes[i].bytesused);
        }
...
static ExynosVideoBuffer *MFC_Decoder_Dequeue_Outbuf(void *pHandle)
{
    ...
    /*fprintf(stderr,"\nSRI-D: plane[0] size:[%d] offset:[%d] , plane[1] size:[%d] offset:[%d]",buf.m.planes[0].bytesused,buf.m.planes[0].data_offset, buf.m.planes[1].bytesused,buf.m.planes[1].data_offset);*/
